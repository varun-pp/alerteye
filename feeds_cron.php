		 ...
		 foreach($categories as $category)
		 {
			   ...
			   if(isset($feed_alerts) && count($feed_alerts) > 0)
			   {
			        ...
			        $email_entire_data .= '<tr>
											  <td style="padding-top:5px; padding-bottom: 2px; font-weight: bold; font-size: 25px; line-height: 24px; font-family: Arial; color: #211a0f;">'.$feed_category_name.'</td>
										  </tr>';
				    $email_entire_data .= '<tr>
			        ...
			        foreach($feed_alerts as $alert)
			        {
						  ...
						  if(isset($new_feeds) && count($new_feeds) > 0)
						  {
								...
								foreach($new_feeds as $feeds)
								{
									  ...
									  $email_data .= '<tr>
														  <td style="padding-bottom: 4px; font-weight: normal; font-size: 11px; line-height: 14px;  font-family: Arial; color: #757067; color:'.$color.'"><span style="color:#000000;padding-right:5px;">'.$count.'.</span>'.$feed_content.'</td>
													  </tr>
 		  ...
 		  if($email_entire_data != '')
		  {
				...
				$url = $_SERVER['HTTP_HOST']."/emailer-tech.html";
				$ch = curl_init();
				curl_setopt ($ch, CURLOPT_URL, $url);
				curl_setopt ($ch, CURLOPT_CONNECTTIMEOUT, 5);
				curl_setopt ($ch, CURLOPT_RETURNTRANSFER, true);
